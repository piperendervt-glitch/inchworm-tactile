...
class LocalNCA:
    ...
    def __init__(self, weights=None):
        ...
        self.state = [[math.cos(-i * 1.0), math.sin(-i * 1.0), 0., 0.] for i in range(N)]
    ...
    def step(self, belly, head):
        ...
        for i in range(N):
            ...
            phase = math.atan2(old[i][1], old[i][0]) + a
            # Preserve a traveling wave instead of synchronizing all joints.
            for j in (i - 1, i + 1):
                if 0 <= j < N:
                    phase += coupling * math.sin(math.atan2(old[j][1], old[j][0]) + (j - i) - math.atan2(old[i][1], old[i][0])) / len(neighbors)
        ...
        self.state = new
        pitch = [max(-.9, min(.9, amplitude * s[1] + .18 * s[2])) for s in new]
...
class World:
    ...
    def geometry(self):
        # Absolute segment pitches approximate vertical bending; no roll DOF.
        local = [(0., 0.)]
        for pitch in self.angles[:5]:
            local.append((local[-1][0] + LENGTH * math.cos(pitch), local[-1][1] + LENGTH * math.sin(pitch)))
        floor = min(z for _, z in local)
        return [(self.x + u * math.cos(self.heading), self.y + u * math.sin(self.heading), z - floor + .006) for u, z in local]
    ...
    def step(self, manual=None):
        if self.hp <= 0:
            return None
        # This exact observation is what generates the action recorded on this row.
        observation = self.belly[:] + self.head[:]
        self.targets = self.controller.step(self.belly, self.head) if manual is None else list(manual)
        old = self.nodes
        self.angles = [v + max(-.06, min(.06, t - v)) for v, t in zip(self.angles, self.targets)]
        candidate = self.geometry()
        # Quasi-static ground anchoring: low segments resist sliding more strongly.
        friction = [math.exp(-min(old[i][2], old[i + 1][2]) / .012) for i in range(N)]
        shifts = [((old[i][0] + old[i + 1][0] - candidate[i][0] - candidate[i + 1][0]) / 2,
                   (old[i][1] + old[i + 1][1] - candidate[i][1] - candidate[i + 1][1]) / 2) for i in range(N)]
        dx = sum(f * d[0] for f, d in zip(friction, shifts)) / sum(friction)
        dy = sum(f * d[1] for f, d in zip(friction, shifts)) / sum(friction)
        blocked = any(o['kind'] == 'obstacle' and math.hypot(candidate[-1][0] + dx - o['x'], candidate[-1][1] + dy - o['y']) < o['radius'] and candidate[-1][2] < o['height'] for o in self.objects)
        if not blocked:
            self.x += dx
            self.y += dy
        self.heading += self.angles[5] * abs(dx * math.cos(self.heading) + dy * math.sin(self.heading)) / .12
        self.nodes = self.geometry()
        self.sense()
        self.tick += 1
        return [self.tick, self.tick * DT] + observation + self.targets + self.angles + [self.hp] + list(self.nodes[-1]) + [self.angles[4], self.heading + self.angles[5], 0.]
...
class Recorder:
    def __init__(self, path, world):
        ...
        self.file.write('# ' + json.dumps({'schema': 1, 'hz': 30, 'observation': 'pre_action_delayed_brightness', 'state': 'post_action', 'config': world.config, 'weights': world.controller.weights, 'initial_tick': world.tick, 'initial_objects': world.objects}) + '\n')
        self.writer = csv.writer(self.file)
        self.writer.writerow(['tick', 'time_s'] + [f'belly_{i}' for i in range(45)] + [f'head_{i}' for i in range(9)] + [f'target_{i}_rad' for i in range(6)] + [f'actual_{i}_rad' for i in range(6)] + ['hp', 'head_x_m', 'head_y_m', 'head_z_m', 'head_pitch_rad', 'head_yaw_rad', 'head_roll_rad'])
