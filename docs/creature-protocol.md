...
| `k` | int | 0..3 | 接触相手種別: 0 なし / 1 壁・構造物・地面 / 2 プレイヤー / 3 他個体 |
...
| `speed` | 実際の水平速度 m/s |
| `eating` | `k == 2` のセルが 1 つ以上ある間 true。捕食ダメージは Body が与える |
| `cells` | 16 要素。省略不可 |
...
| 捕食（プレイヤーへの被害） | Body | `eating: true` を observe で通知。Brain はこれでエネルギー回復 |
| ミッション終了 | Body | `bye` |
