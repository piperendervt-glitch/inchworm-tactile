...
class NewbornController:
    VERSION=3
    INPUTS=45  # 9 belly + 9 head + 2 joint + 9 support + 4 internal + 4 self + 8 neighbors
    HIDDEN=4
    ...
    def __init__(self,seed=17):
        ...
        self.output_weights=tuple(tuple(rng.gauss(0,.35/2) for _ in range(4)) for _ in range(3))
        ...
        self.grips=[0.,0.]
        self.outputs=[[0.,0.,0.] for _ in range(5)]
        self.training_steps=0

    def step(self,obs):
        if not isinstance(obs,Observation):raise TypeError('Observation required')
        if tuple(map(len,(obs.belly,obs.head,obs.joint_touch,obs.support_touch)))!=(45,9,10,18):
            raise ValueError('Expected 45 belly, 9 head, 10 joint-touch and 18 support-touch values')
        ...
        for i in range(5):
            ...
            joint=list(obs.joint_touch[i*2:(i+1)*2])
        ...
        for i,hidden in enumerate(new):
            ...
            self.outputs[i]=[.9*math.tanh(raw[0]),1/(1+math.exp(-raw[1])),.8*math.tanh(raw[2])]
        # Hardware masks: only the two terminal modules have support actuators;
        # yaw exists only on the head. No support timing or contact gate is imposed.
        self.grips=[self.outputs[0][1],self.outputs[4][1]]
        return [output[0] for output in self.outputs]+[self.outputs[4][2]]
